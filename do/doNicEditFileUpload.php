...
define('NICUPLOAD_URI', PATH."upload");   // Set the URL (relative or absolute) to
// the directory defined above

$nicupload_allowed_extensions = array('jpg','jpeg','png','gif','bmp','pdf','doc','docx','xls','xlsx','ppt','pptx','zip','txt','rtf','mp3','ogg');
...
if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
    ...
    $status['name'] = $_FILES['nicImage']['name'];
...
function nicupload_output($status, $showLoadingMsg = false) {
    $script = '
        try {
            '.(($_SERVER['REQUEST_METHOD']=='POST') ? 'top.' : '').'nicFileUploadButton.statusCb('.json_encode($status).');
        } catch(e) { alert("Welcome to the fail: "+e.message); }
    ';
